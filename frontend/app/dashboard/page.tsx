...
export default async function Dashboard() {
  ...
  const [userResponse,datasetResponse,summaryResponse]=await Promise.all([fetch(`${apiUrl}/auth/me`,{headers,cache:"no-store"}),fetch(`${apiUrl}/datasets/status`,{headers,cache:"no-store"}),fetch(`${apiUrl}/dashboard/summary`,{headers,cache:"no-store"})]);
  ...
  const user=await userResponse.json() as {full_name:string;email:string}; const summary=await summaryResponse.json() as DashboardSummary;
  const initials=user.full_name.split(" ").map(part=>part[0]).join("").slice(0,2).toUpperCase(); const margin=summary.revenue?Math.round(summary.profit/summary.revenue*100):0; const revenueTrend=summary.revenue_overview.map(item=>item.value); const supportTrend=revenueTrend.map((value,index)=>Math.max(0,Math.round(value/(index+1)))); const today=new Intl.DateTimeFormat("en-IN",{day:"numeric",month:"long",year:"numeric"}).format(new Date());
