...
type MetricCardProps = {
  ...
  value: string;
  detail: string;
  icon: "revenue" | "profit" | "employees" | "support" | "cash";
  color: string;
  trend: number[];
  positive?: boolean;
};
...
export function MetricCard({ label, value, detail, icon, color, trend, positive = true }: MetricCardProps) {
  const Icon = icons[icon];
  const TrendIcon = positive ? ArrowUp : ArrowDown;
  const chartData = trend.map((item, index) => ({ index, value: item }));
  ...
  return <article className="panel-glow min-w-0 rounded-xl border border-slate-800/90 bg-[#101722] p-4">
    ...
    <p className={`mt-2 flex items-center gap-1 text-xs ${positive ? "text-emerald-400" : "text-rose-400"}`}><TrendIcon size={12} /> {detail} <span className="text-slate-500">vs prior period</span></p>
