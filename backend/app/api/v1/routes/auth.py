...
@router.post("/register", response_model=MessageResponse, status_code=status.HTTP_202_ACCEPTED, summary="Send a real registration OTP")
def register(payload: RegisterRequest, db: Session = Depends(get_db)) -> dict:
    try: return OtpService(db).request_registration(email=str(payload.email), full_name=payload.full_name, organization_name=payload.organization_name or f"{payload.full_name}'s Organization", password=payload.password)
...
def verify_registration(payload: OtpVerifyRequest, db: Session = Depends(get_db)) -> UserResponse:
...
def verify_otp_login(payload: OtpVerifyRequest, db: Session = Depends(get_db)) -> TokenResponse:
    return TokenResponse(access_token=OtpService(db).verify_login(str(payload.email),payload.otp,payload.remember))
